...
const ProductUploadPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        var snapshot = await getDoc(userDoc);
        ...
        const utilsDoc = doc(db, "Utils", "PUBLIC_ID");
        snapshot = await getDoc(utilsDoc);

        if (!snapshot.exists()) {
            throw new Error("Utils document does not exist.");
        }

        const utilsData = snapshot.data();
        if (!utilsData || utilsData.id === undefined) {
            throw new Error("Public ID data is missing.");
        }
        
        const newID = utilsData.id + 1;
    ...
    console.error("Failed during the product upload process:", error);
    alert(error.message);
}
};
